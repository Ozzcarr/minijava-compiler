...
class BasicBlock {
   ...
   public:
    ...
    BasicBlock(const std::string &name) : name(name), trueExit(-1), falseExit(-1) {}
...
class ControlFlowGraph {
   ...
   private:
    ...
    // void traverseClassDeclList(Node *node);
    // void traverseClassDecl(Node *node);
    // void traverseMethodDecl(Node *node);
    void traverseStatementList(Node *node);
    void traverseStatement(Node *node);
    void traversePrintStatement(Node *node);
