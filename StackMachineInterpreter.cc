...
bool StackMachineInterpreter::loadBytecode(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (colonPos != std::string::npos) {
            ...
            OpCode opcode;
            if (opcodeName == "iload") opcode = OpCode::ILOAD;
            else if (opcodeName == "iconst") opcode = OpCode::ICONST;
            else if (opcodeName == "istore") opcode = OpCode::ISTORE;
            else if (opcodeName == "iadd") opcode = OpCode::IADD;
            else if (opcodeName == "isub") opcode = OpCode::ISUB;
            else if (opcodeName == "imul") opcode = OpCode::IMUL;
            else if (opcodeName == "idiv") opcode = OpCode::IDIV;
            else if (opcodeName == "ilt") opcode = OpCode::ILT;
            else if (opcodeName == "igt") opcode = OpCode::IGT;
            else if (opcodeName == "ieq") opcode = OpCode::IEQ;
            else if (opcodeName == "iand") opcode = OpCode::IAND;
            else if (opcodeName == "ior") opcode = OpCode::IOR;
            else if (opcodeName == "inot") opcode = OpCode::INOT;
            else if (opcodeName == "goto") opcode = OpCode::GOTO;
            else if (opcodeName == "iffalsegoto") opcode = OpCode::IFFALSEGOTO;
            else if (opcodeName == "invokevirtual") opcode = OpCode::INVOKEVIRTUAL;
            else if (opcodeName == "ireturn") opcode = OpCode::IRETURN;
            else if (opcodeName == "print") opcode = OpCode::PRINT;
            else if (opcodeName == "stop") opcode = OpCode::STOP;
            else {
    ...
    if (mainClass.empty()) {
        ...
        for (const auto& [methodName, _] : methods) {
            ...
            if (dotPos != std::string::npos && 
                methodName.substr(dotPos + 1) == "main") {
    ...
    switch (opcode) {
        case OpCode::ILOAD: {
            ...
            if (argument == "true") {
                ...
                operandStack.push(0); // Default to 0 for undefined variables
            } else {
        ...
        case OpCode::ICONST: {
            ...
            operandStack.push(std::stoi(argument));
            programCounter++;
            break;
        }
        ...
        case OpCode::ISTORE: {
            ...
            localVariables[argument] = operandStack.top();
            operandStack.pop();
            programCounter++;
            break;
        }
        ...
        case OpCode::IRETURN: {
            ...
            int returnValue = operandStack.top();
            operandStack.pop();
        ...
        if (dotPos != std::string::npos) {
            ...
            for (const auto& [name, _] : methods) {
                ...
                if (nameDotPos != std::string::npos && 
                    name.substr(nameDotPos + 1) == methodPart) {
...
int StackMachineInterpreter::getVariable(const std::string& name) const {
